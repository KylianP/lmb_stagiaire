...
function stock_etat_imprimer () {			
		...
		if ($F('id_stock_l').length >1 || $F('id_stock_l').length == 0 || ($F('id_stock_l').length == 1 && $F('id_stock_l')== "")) {
			alerte.alerte_erreur ('Erreur de saisie', 'Vous devez selectionner un seul lieu de stockage pour pouvoir imprimer','<input type="submit" id="bouton0" name="bouton0" value="Ok" />');
		} else {
