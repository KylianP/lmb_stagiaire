...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td>
			Afficher les tarifs 
			<input  name="taxation" type="radio" id="taxation_ht" value="HT" <?php if ($_REQUEST["app_tarifs"] == "HT") {?> checked="checked"<?php }?>>HT
			<input  name="taxation" id="taxation_ttc" type="radio" value="TTC" <?php if ($_REQUEST["app_tarifs"] == "TTC") {?> checked="checked"<?php }?>>TTC
			
		</td>
		<td>&nbsp;</td>
		<td style="width:30%"><span class="labelled_text" <?php if (!$GESTION_STOCK) {?> style="display:none"<?php }?>>Lieux de stockage:
			<select name="id_stock_l_a" id="id_stock_l_a" class="">
				<?php 
		...
		<td style="width:25%"><span class="labelled_text">Tarifs:</span>
			<select name="id_tarif_l_a" id="id_tarif_l_a" class="">
			<?php
...
<table  cellspacing="0" id="tableresult">
	<tr class="colorise0">
		<td style="width:20%">
			<a href="#"  id="order_ref">R&eacute;f&eacute;rence
			</a>
		</td>
		<td style="width:40%">
			<a href="#"  id="order_lib">Libell&eacute;
			</a>
		</td>
		...
		<td style="width:15%; text-align:center">
			<a href="#"  id="order_tarif" >Tarif
			<?php if ($_REQUEST["app_tarifs"] == "HT") {echo "HT";} else { echo "TTC";}?>
			</a>
		</td>
		<td style="width:5%"></td>
	</tr>
...
foreach ($fiches as $fiche) {
	...
	if ($GESTION_STOCK) { 
		...
		<div id="aff_tarif_ht_<?php echo $colorise;?>" style="display:<?php if ($_REQUEST["app_tarifs"] != "HT") { echo "none";}?>">
		<?php	
	...
	<div id="aff_tarif_ttc_<?php echo $colorise;?>" style="display:<?php if ($_REQUEST["app_tarifs"] != "TTC") { echo "none";}?>">
		<?php	
		foreach ($fiche->tarifs as $tarif) {
			if (count($fiche->tarifs) == 1) {
...
Event.observe("order_ref", "click",  function(evt){Event.stop(evt); $('orderby_a').value='ref_interne'; $('orderorder_a').value='<?php if ($form['orderorder']=="ASC" && $form['orderby']=="ref_interne") {echo "DESC";} else {echo "ASC";}?>'; page.catalogue_recherche_simple();}, false);
Event.observe("order_lib", "click",  function(evt){Event.stop(evt); $('orderby_a').value='lib_article'; $('orderorder_a').value='<?php if ($form['orderorder']=="ASC" && $form['orderby']=="lib_article") {echo "DESC";} else {echo "ASC";}?>'; page.catalogue_recherche_simple();}, false);
Event.observe("order_tarif", "click",  function(evt){Event.stop(evt); $('orderby_a').value='lib_article'; $('orderorder_a').value='<?php if ($form['orderorder']=="ASC" && $form['orderby']=="lib_article") {echo "DESC";} else {echo "ASC";}?>'; page.catalogue_recherche_simple();}, false);


Event.observe('taxation_ttc', "click", function(evt){
 $("app_tarifs").value = "TTC";
 for (i=1; i <= <?php echo $colorise;?>; i++) {
 $("aff_tarif_ttc_"+i).show();
 $("aff_tarif_ht_"+i).hide();
 $("order_tarif").innerHTML = "Tarif TTC";
 }
});
Event.observe('taxation_ht', "click", function(evt){
 ...
 $("order_tarif").innerHTML = "Tarif HT";
 }
});


Event.observe('id_stock_l_a', "change", function(evt){$('id_stock').value=$('id_stock_l_a').value; page.catalogue_recherche_avancee();});
Event.observe('id_tarif_l_a', "change", function(evt){$('id_tarif').value=$('id_tarif_l_a').value; page.catalogue_recherche_avancee();});
