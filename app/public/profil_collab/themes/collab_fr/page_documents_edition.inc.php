...
if (isset($_REQUEST['id_type_doc']) || isset($_REQUEST["fonction_generer"])) {
	...
	document.location.hash = escape("documents_edition.php?ref_doc=<?php echo htmlentities($document->getRef_doc())?>");
	<?php
}
?>
document_accept_reglement = <?php if($document->getACCEPT_REGMT() == 0){echo 'false';}else{echo 'true';}?>;
...
<!--	<li id="doc_menu_3">
	...
	<li id="doc_menu_5">
		<a href="#" id="menu_5" class="menu_unselect" style=" <?php if ($document->getDescription () != "") { echo "color:#FF0000";}?>">Note</a>
	</li>
</ul>
</div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="document_box_head">
	<tr>
		...
		<td style="width:280px; padding-left:3px;" class="doc_head_bold">
...
foreach ($liste_contenu as $contenu) {
	...
	if ($contenu->ref_doc_line_parent == "") {
		if ($indentation_contenu != 0) { echo "</li>";}
		?>
			...
			<table style="width:185px">
				...
				<tr>
					...
					<div id="distinct_intit_tva"<?php
						if (!$ASSUJETTI_TVA && ($id_type_doc == $DEVIS_CLIENT_ID_TYPE_DOC || $id_type_doc == $COMMANDE_CLIENT_ID_TYPE_DOC || $id_type_doc == $LIVRAISON_CLIENT_ID_TYPE_DOC || $id_type_doc == $FACTURE_CLIENT_ID_TYPE_DOC) && $document->getMontant_tva () == 0) { echo 'style="display:none;" ';}
						?>></div>
					</td>
					<td style="text-align:right">
				...
				<tr>
					...
					<div id="d_toto_tva" <?php
						...
						?>></div>
						<?php
						if (!$ASSUJETTI_TVA && ($id_type_doc == $DEVIS_CLIENT_ID_TYPE_DOC || $id_type_doc == $COMMANDE_CLIENT_ID_TYPE_DOC || $id_type_doc == $LIVRAISON_CLIENT_ID_TYPE_DOC || $id_type_doc == $FACTURE_CLIENT_ID_TYPE_DOC) && $document->getMontant_tva () == 0) { echo 'n/a';}
						?>
					</td>
				</tr>
			</table>
...
Afficher  les prix <input name="prix_afficher" id="prix_afficher_ht" type="radio" value="HT" <?php if ($document->getApp_tarifs() == "HT" || $document->getApp_tarifs() == "") {echo 'checked="checked"';}?> />HT <input name="prix_afficher" id="prix_afficher_ttc" type="radio" value="TTC" <?php if ($document->getApp_tarifs() == "TTC") {echo 'checked="checked"';}?> />TTC<br />
</div>
