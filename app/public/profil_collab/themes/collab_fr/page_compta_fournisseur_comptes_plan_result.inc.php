...
//
	...
	$barre_nav .= barre_navigation($nb_fiches, $form['page_to_show'], 
                                       ...
                                       $debut, $cfg_nb_pages,
                                       'page_to_show',
																			 'page.compta_fournisseur_comptes_plan()');
...
foreach ($fiches as $fiche){
	?>
		<tr id="line_compte_comptable" style="">
			...
			<span style="text-decoration:underline; cursor:pointer" id="numero_compte_compta_fournisseur_<?php echo $fiche->ref_fournisseur;?>"><?php if ($fiche->defaut_numero_compte) { echo $fiche->defaut_numero_compte;} else if($fiche->categ_defaut_numero_compte) { echo $fiche->categ_defaut_numero_compte;} else { echo $DEFAUT_COMPTE_TIERS_ACHAT;}?></span>
			</td>
			<td>&nbsp;
			</td>
			<td>
			<span id="aff_numero_compte_compta_fournisseur_<?php echo $fiche->ref_fournisseur;?>">
			<?php if ($fiche->defaut_numero_compte) { echo $fiche->defaut_lib_compte;} else if($fiche->categ_defaut_numero_compte) { echo $fiche->categ_defaut_lib_compte;} else {  $lcpt = new compta_plan_general($DEFAUT_COMPTE_TIERS_ACHAT); echo $lcpt->getLib_compte();}?>
			</span>
			<script type="text/javascript">
			
			Event.observe('numero_compte_compta_fournisseur_<?php echo $fiche->ref_fournisseur;?>', 'click',  function(evt){
				ouvre_compta_plan_mini_moteur(); 
				charger_compta_plan_mini_moteur ("compte_plan_comptable_search.php?retour_value_id=numero_compte_compta_fournisseur_<?php echo $fiche->ref_fournisseur;?>&retour_lib_id=aff_numero_compte_compta_fournisseur_<?php echo $fiche->ref_fournisseur;?>&indent=numero_compte_compta_fournisseur_<?php echo $fiche->ref_fournisseur;?>&ref_contact=<?php echo $fiche->ref_fournisseur;?>&num_compte=<?php if($fiche->categ_defaut_numero_compte) { echo $fiche->categ_defaut_numero_compte;} else { echo $DEFAUT_COMPTE_TIERS_ACHAT;}?>");
				Event.stop(evt);
			},false); 
