	...
	<tr class="">
		<td colspan="3">
									...
									<table cellpadding="0" cellspacing="0" border="0" style="width:380px;">
										...
										<tr style=" line-height:24px; height:24px;">
											...
											<td style="width:250px;">
												...
												<option value="">Non transmise</option>
												<?php 
										...
										<tr style=" line-height:24px; height:24px;">
											...
											<td style="width:250px;">
												<a id="head_date_next_relance" class="modif_input2">&nbsp;<?php 
													if ($document->getdate_next_relance ()!=0000-00-00) {
														echo date_Us_to_Fr ($document->getdate_next_relance ());
													}?></a>
												<input type="text" value="<?php 
													if ($document->getdate_next_relance ()!=0000-00-00) {
														echo date_Us_to_Fr ($document->getdate_next_relance ());
													}?>" id="date_next_relance" name="date_next_relance" style="display:none" /> 
													
												<input type="hidden" value="<?php 
													if ($document->getdate_next_relance ()!=0000-00-00) {
														echo date_Us_to_Fr ($document->getdate_next_relance ());
													}?>" id="date_next_relance_old" name="date_next_relance_old"/>
											</td>
										</tr>
									</table>
					</td>
									...
									<table cellpadding="0" cellspacing="0" border="0" style="width:100%;">
										...
										<tr>
											...
											if ($document->getId_etat_doc () == 16) {
												?>
											<img src="<?php echo $DIR.$_SESSION['theme']->getDir_theme()?>images/bt_fac_aregler.gif" id="facture_a_regler" style="cursor:pointer"/>
			...
			<script type="text/javascript">
				...
				Event.observe("date_next_relance", "blur", function(evt){
					if ($("date_next_relance").value != $("date_next_relance_old").value) {
						datemask (evt);
						$("date_next_relance_old").value = $("date_next_relance").value;
						maj_date_next_relance ("date_next_relance");
						$("head_date_next_relance").innerHTML = $("date_next_relance").value + "&nbsp;";
					}
					$("date_next_relance").hide();
					$("head_date_next_relance").show();
				}, false);
				Event.observe("head_date_next_relance", "click", function(evt){
					Event.stop(evt);
					$("head_date_next_relance").hide();
					$("date_next_relance").show();
					$("date_next_relance").focus();
				}, false);
