...
<p class="titre" id="titre_crea_art">Copie d'un article </p>
<div>
	...
	<table style="width:100%">
		...
		<tr>
			...
			<span style="position:relative; width:100%;" class="simule_champs" id="liste_de_categorie_pour_article">
				...
				<tr>
				<td>
				<span id="lib_art_categ" style=" float:left; height:18px; margin-left:3px; line-height:18px;"><?php echo ($article->getLib_art_categ ());?></span>
				</td>
				<td>
				...
				<?php
					...
					<tr id="tr_<?php echo ($art_categ->ref_art_categ)?>" class="list_art_categs"><td width="5px">
									...
									if ($i==$art_categ->indentation) {
										if (key($list_art_categ)!="") {
											if ($art_categ->indentation < current($list_art_categ)->indentation) {
												?><a href="#" id="link_extend_<?php echo $art_categ->ref_art_categ?>">
												...
												<img src="<?php echo $DIR.$_SESSION['theme']->getDir_theme()?>images/<?php if ($DEFAUT_LEVEL_CATEG_AFFICHED > $i) {echo "extend";} else {echo "collapse";}?>.gif" width="14px" id="collapse_<?php echo $art_categ->ref_art_categ?>" style="display:none"/></a>
												<script type="text/javascript">
												Event.observe("link_extend_<?php echo $art_categ->ref_art_categ?>", "click",  function(evt){Event.stop(evt); Element.toggle('div_<?php echo $art_categ->ref_art_categ?>') ; Element.toggle('extend_<?php echo $art_categ->ref_art_categ?>'); Element.toggle('collapse_<?php echo $art_categ->ref_art_categ?>');}, false);
												</script>
												<?php
											}
											else 
						...
						<?php echo ($art_categ->lib_art_categ)?>
						</a>
						</td>
					</tr>
					</table>
		...
		<tr>
			...
			<input type="hidden" name="modele" id="modele" value="<?php echo $art_categs->getModele()?>" /></td>
			<td rowspan="2" style="width:25%"><textarea name="lib_article" class="classinput_xsize" id="lib_article"><?php echo ($article->getLib_article ());?></textarea></td>
			<td>&nbsp;</td>
			<?php 
		...
		<tr>
			...
			if($GESTION_LIB_TICKET){
				...
				<td>
					<input type="text" name="lib_ticket" id="lib_ticket" value="<?php echo ($article->getLib_ticket ());?>"  class="classinput_xsize"/>			</td>
				<td>&nbsp;</td>
				<?php 
			}else{ 
				?>
		...
		<tr>
			...
			<td colspan="4">
				<textarea name="desc_courte" rows="4" class="classinput_xsize" style="width:100%" id="desc_courte"><?php echo ($article->getDesc_courte ());?></textarea>			
			</td>
			<td>&nbsp;</td>
		</tr>
		<tr>
...
<?php include $DIR.$_SESSION['theme']->getDir_theme()."page_catalogue_articles_duplicate_tarifs.inc.php" ?>
</div>
...
<div class="articletview_corps"  id="gestion_info"  style="OVERFLOW-Y: auto; OVERFLOW-X: auto; width:100%; display:none">
	<div id="gestion_info_under">
		...
		<select name="gestion_sn" id="gestion_sn" class="classinput_hsize">
			...
			?>
			<td>&nbsp;</td>
			<td class="labelled_text">Code barre: </td>
			<td>
			<input type="text" name="a_code_barre" id="a_code_barre" value="" onfocus="return false"  class="classinput_xsize"/>
			<input type="hidden" name="serialisation_code_barre" id="serialisation_code_barre" value="0" /></td>
			<td>&nbsp;</td>
		</tr>
		...
		<tr>
			...
			<td colspan="3">
		<div id="modele_info">
			<?php include $DIR.$_SESSION['theme']->getDir_theme()."page_catalogue_articles_duplicate_modele_".$art_categs->getModele().".inc.php" ?>
		</div>
			</td>
			<td colspan="2">
...
<?php include $DIR.$_SESSION['theme']->getDir_theme()."page_catalogue_articles_duplicate_categ_caract.inc.php" ?>
</div>
...
<div class="articletview_corps" id="liaison_info"  style="OVERFLOW-Y: auto; OVERFLOW-X: auto; width:100%; display:none">
	<div id="liaison_info_under" style="margin-left:5%; margin-right:5%;">
		<input type="text" name="serialisation_liaison" id="serialisation_liaison" value="0" />
		<?php foreach ($liaisons_type_liste as $liaison_type) { 
			...
			<?php foreach ($liaisons_vers_autres_articles as $article_liaison){ ?>
				...
				construire_ligne_liaison_article_duplication(parseInt($("serialisation_liaison").value), "vers", "REF_NOUVEL_ARTICLE", "<?php echo $article_liaison["article_lie"]->getRef_article();?>", <?php echo $liaison_type->getId_liaison_type(); ?>, "<?php echo $article_liaison["article_lie"]->getLib_article();?>", <?php echo $article_liaison["ratio"]; ?>);
			<?php } ?>
			</script>
			...
			<?php foreach ($liaisons_depuis_autres_articles as $article_liaison){ ?>
				...
				construire_ligne_liaison_article_duplication(parseInt($("serialisation_liaison").value), "depuis", "REF_NOUVEL_ARTICLE", "<?php echo $article_liaison["article_lie"]->getRef_article();?>", <?php echo $liaison_type->getId_liaison_type(); ?>, "<?php echo $article_liaison["article_lie"]->getLib_article();?>", <?php echo $article_liaison["ratio"]; ?>);
			<?php } ?>
			</script>
		<?php } ?>
						...
						<script type="text/javascript">
							...
							Event.observe("nouvelle_liaison_vers_valider", "click",  function(evt){
								...
								if($("nouvelle_liaison_ref_article_lie").value != "" && $("nouvelle_liaison_lib_article_lie").value != ""){
									...
									link_article_to_article_duplication_vers(parseInt($("serialisation_liaison").value), $("nouvelle_liaison_type_vers_selected").options[$("nouvelle_liaison_type_vers_selected").selectedIndex].value, "REF_NOUVEL_ARTICLE", $("nouvelle_liaison_ref_article_lie").value, $("nouvelle_liaison_lib_article_lie").value, ratio);
									$("nouvelle_liaison_lib_article_lie").value = "";
									$("nouvelle_liaison_ref_article_lie").value = "";
									$("nouvelle_liaison_ratio_vers").value = "1";
									$("nouvelle_liaison_type_vers_selected").selectedIndex = 0;
						...
						<script type="text/javascript">
							...
							Event.observe("nouvelle_liaison_depuis_valider", "click",  function(evt){
								...
								if($("nouvelle_liaison_ref_article").value != "" && $("nouvelle_liaison_lib_article").value != ""){
									...
									link_article_to_article_duplication_depuis(parseInt($("serialisation_liaison").value), $("nouvelle_liaison_type_depuis_selected").options[$("nouvelle_liaison_type_depuis_selected").selectedIndex].value, "REF_NOUVEL_ARTICLE", $("nouvelle_liaison_ref_article").value, $("nouvelle_liaison_lib_article").value, ratio);
									$("nouvelle_liaison_lib_article").value = "";
									$("nouvelle_liaison_ref_article").value = "";
									$("nouvelle_liaison_ratio_depuis").value = "1";
									$("nouvelle_liaison_type_depuis_selected").selectedIndex = 0;
...
chemin[1]=Array("1_0", "1_1", "1_2", "1_3", "caract_info", "allowed", true);
chemin[2]=Array("2_0", "2_1", "2_2", "2_3", "gestion_info", "allowed", true);
chemin[3]=Array("3_0", "3_1", "3_2", "3_3", "tarifs_info", "allowed", true);
chemin[4]=Array("4_0", "4_1", "4_2", "4_3", "lot_info", "<?php if($article->getLot ()) {echo 'allowed';} else  {echo 'notallowed';}?>", <?php if ($article->getLot ()) {?>true<?php }else{ ?>false<?php }?>);
chemin[5]=Array("5_0", "5_1", "5_2", "5_3", "images_info", "allowed", true);
chemin[6]=Array("6_0", "6_1", "6_2", "6_3", "liaison_info", "allowed", true);
...
function valide_etape_1() {
	...
	//	Event.observe($("bt_etape_3"), "click", function(){goto_etape (4);});
		...
		Event.observe($("bt_etape_6"), "click", function(evt){Event.stop(evt); valide_create_article();});
	}else {
		for (key in chemin) {
...
//------------------------------------------------------------------------------------
				...
				foreach ($list_art_categ  as $art_categ){
					...
					if ($art_categ->duree_dispo) {
						...
						echo '$("date_fin_dispo").style.display = "";';
						echo '$("infinite_choix").style.display = "none";';
					} else {
