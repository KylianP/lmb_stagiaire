...
<a href="#" id="menu_9" class="menu_unselect" style="text-align: left">Pi�ces jointes</a>
</span>
</div>
<script type="text/javascript">
	...
	<div id="liaison_info_under" style="margin-left:5%; margin-right:5%;">
		...
		<?php
			...
			<?php foreach ($liaisons_vers_autres_articles as $article_liaison){ ?>
				...
				construire_ligne_liaison_article_view(parseInt($("serialisation_liaison").value), "vers", "<?php echo $article->getRef_article();?>", "<?php echo $article_liaison["article_lie"]->getRef_article();?>", <?php echo $liaison_type->getId_liaison_type(); ?>, "<?php echo addslashes($article_liaison["article_lie"]->getLib_article());?>", <?php echo $article_liaison["ratio"]; ?>);
			<?php } ?>
			</script>
			...
			<?php foreach ($liaisons_depuis_autres_articles as $article_liaison){ ?>
				...
				construire_ligne_liaison_article_view(parseInt($("serialisation_liaison").value), "depuis", "<?php echo $article->getRef_article();?>", "<?php echo $article_liaison["article_lie"]->getRef_article();?>", <?php echo $liaison_type->getId_liaison_type(); ?>, "<?php echo addslashes($article_liaison["article_lie"]->getLib_article());?>", <?php echo $article_liaison["ratio"]; ?>);
			<?php } ?>
			</script>
		<?php 
			} ?>
...
Event.observe('menu_1', "click", function(evt){
	page.verify("articles_view_ingo_gene","catalogue_articles_view_info_generales.php?ref_article=<?php echo $article->getRef_article ();?>", "true", "info_gene");
