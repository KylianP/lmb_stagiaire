...
protected function create_pdf_adresse () {
	...
	if (strlen($this->dest_contact->getLib_civ_court())) {
		$this->pdf->Cell (0,4.5, $this->dest_contact->getLib_civ_court() . " " . $this->dest_contact->getNom(), 0 , 2 , "L");
	} else {
		$this->pdf->Cell (0,4.5, $this->dest_contact->getNom(), 0 , 2 , "L");
	}
	$this->pdf->SetXY($decalage_gauche + $marge, $decalage_haut + $marge + 14);
	$this->pdf->SetFont('Arial', '', 9);
	$this->pdf->MultiCell (0,4.5, $this->dest_adresse->getText_adresse (),  2 , "L");
	...
	if ($this->dest_adresse->getPays () != "FRANCE") {
...
protected function create_pdf_objet () {
	...
	$this->pdf->Cell ($this->LARGEUR_TOTALE_CORPS, 5,lmb_strftime('%A %d %B %Y', "fr_FRA", strtotime($this->courrier->getDate_courrier())).",",0,2,"R");
