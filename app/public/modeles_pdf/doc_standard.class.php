...
protected function create_pdf_entete () {
	...
	if($this->montant_ht>=0 || empty($this->LIB_NEG)){
		$this->pdf->Cell (95, 10, $this->lib_type_printed, 0, 0, 'L','true');
	}else{
		$this->pdf->Cell (95, 10, $this->LIB_NEG, 0, 0, 'L','true');
	}
	$this->pdf->SetTextColor(0,0,0);
...
protected function create_pdf_corps () {
	...
	if(!isset($this->REF_ARTICLE) || $this->REF_ARTICLE!="Aucune"){
		$this->pdf->Cell ($this->LARGEUR_COL_REF, 6, $this->ENTETE_COL_REF, 1, 0, 'L','true');
	}
	$this->pdf->Cell ($this->LARGEUR_COL_LIB, 6, $this->ENTETE_COL_DES, 1, 0, 'L','true');
	$this->pdf->Cell ($this->LARGEUR_COL_QTE, 6, $this->ENTETE_COL_QTE, 1, 0, 'C','true');
	if(!isset($this->AFF_PRIX) || $this->AFF_PRIX){
		$this->pdf->Cell ($this->LARGEUR_COL_PRI, 6, $this->ENTETE_COL_PU, 1, 0, 'C','true');
		if ($AFF_REMISES) {
			$this->pdf->Cell ($this->LARGEUR_COL_REM, 6, $this->ENTETE_COL_REM, 1, 0, 'C','true');
		}
		$this->pdf->Cell ($this->LARGEUR_COL_PRI, 6, $this->ENTETE_COL_PT, 1, 0, 'C','true');
		$this->pdf->Cell ($this->LARGEUR_COL_TVA, 6, $this->ENTETE_COL_TVA, 1, 0, 'C','true');
	}
	$this->pdf->SetTextColor(0,0,0);
