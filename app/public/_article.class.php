...
function __construct($ref_article = "") {
	...
	$query = "SELECT ref_oem, ref_interne, lib_article, lib_ticket, desc_courte, desc_longue,
...
final public function create ($infos_generales, $infos_modele, $caracs, $formuls_tarifs, $composants, $liaisons, $ref_article = "") {
	global $CONFIG_DIR;
	global $bdd;
	...
	$query = "INSERT INTO articles
							(ref_article, ref_oem, ref_interne, lib_article, lib_ticket, desc_courte, desc_longue,
							 ...
							 lot, variante, gestion_sn, date_debut_dispo, date_fin_dispo, dispo, date_creation, date_modification)
						VALUES ('".$this->ref_article."', '".addslashes($this->ref_oem)."',
										...
										'".$this->date_debut_dispo."', '".$this->date_fin_dispo."', '".$this->dispo."', NOW(), NOW() ) ";
	$bdd->exec ($query);
...
final public function modification_view_0 ($infos_generales) {
	...
	// Envoi EDI
	if($this->variante != 1){
		edi_event(113,$this->ref_article);
	}
...
final function findVariantesStock(){
	global $bdd;
...
// Ajout d'une caractéristique
public function maj_carac ($ref_carac, $valeur) {
	global $bdd;
...
function add_taxe ($id_taxe, $code_taxe, $info_calcul) {
	...
	$taxe_file = $DIR."taxes/taxe_".$code_taxe.".inc.php";
	if (!is_file($taxe_file)) {
		$erreur = "Le fichier ".$taxe_file." permettant le calcul du montant de la taxe ".$id_taxe." n'est pas défini";
		alerte_dev ($erreur);
	}
	include ($taxe_file);
	...
	// *************************************************
	// Montant de la taxe
	if (!isset($montant_taxe)) {
		$erreur = "Le calcul du montant de la taxe ".$id_taxe." ne s'effecue pas correctement.";
		alerte_dev ($erreur);
	}
	...
	$resultat = $bdd->query($query);
	if ($tmp = $resultat->fetchObject()) {
		$query_del = "DELETE FROM articles_taxes WHERE ref_article = '".$this->ref_article."' && id_taxe = '".$id_taxe."'";
		$bdd->exec ($query_del);
	}
	// *************************************************
	// Insertion dans la base
...
function maj_montant_taxe ($id_taxe, $montant_taxe) {
	global $bdd;
	global $DIR;
...
public function add_image ($lib_file) {
	...
	$query = "SELECT MAX(id_image) id_image FROM images_articles WHERE lib_file = '".$lib_file."' ";
	$resultat = $bdd->query ($query);
	$tmp = $resultat->fetchObject ();
	$id_image = $tmp->id_image;
	...
	if($this->variante != 1 ){
		edi_event(114,$this->ref_article,$lib_file);
	}
