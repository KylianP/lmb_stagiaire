...
function load_facture_nonreglees (id_client_categ, id_niveau_relance) {
		var AppelAjax = new Ajax.Updater(
									...
									{
									parameters: {id_client_categ: id_client_categ, id_niveau_relance: id_niveau_relance },
									evalScripts:true, 
...
function load_facture_nonreglees_fournisseur (id_fournisseur_categ) {
		var AppelAjax = new Ajax.Updater(
									...
									{
									parameters: {id_fournisseur_categ: id_fournisseur_categ },
									evalScripts:true, 
									onLoading:S_loading, onException: function () {S_failure();},
									onComplete: function() {
															H_loading (); 
...
function load_livraison_nonfacturees (id_stock, orderorder, orderby) {
		var AppelAjax = new Ajax.Updater(
									...
									{
									parameters: {id_stock: id_stock, orderorder: orderorder, orderby: orderby },
									evalScripts:true, 
									onLoading:S_loading, onException: function () {S_failure();},
									onComplete: function() {
															H_loading (); 
...
function maj_niveau_relance (ref_doc, id_niveau_relance) {
	var AppelAjax = new Ajax.Request( 
									...
									{
									parameters: {ref_doc: ref_doc, id_niveau_relance : id_niveau_relance},
									evalScripts:true, 
									onLoading:S_loading, onException: function () {S_failure();}, 
									onComplete: function() {
															H_loading(); 
...
function unlink_doc_to_reglement (ref_doc, ref_reglement, id_tag, maj_ref_doc, maj_ref_contact) {
	var AppelAjax = new Ajax.Request( 
									...
									{
									parameters: {ref_doc: ref_doc, ref_reglement : ref_reglement, id_tag: id_tag, maj_ref_doc: maj_ref_doc, maj_ref_contact: maj_ref_contact},
									evalScripts:true, 
									onLoading:S_loading, onException: function () {S_failure();}, 
...
function	etat_caisse_result () {
	reglements_modes = $("id_reglement_mode").getValue().join(", ");
		var AppelAjax = new Ajax.Updater(
									...
									encoding:     'UTF-8',
									parameters: { recherche: '1', id_compte_caisse: $("choix_id_caisse").value, id_reglement_mode: reglements_modes, page_to_show: $("page_to_show_s").value , date_debut: $("date_debut").value, date_fin: $("date_fin").value},
									evalScripts:true, 
									onLoading:S_loading, onException: function () {S_failure();}, 
...
function calcul_controle_caisse () {
	...
	if ($("toto_esp_theorique").innerHTML != "") { toto_theo += toto_esp_theo = parseFloat($("toto_esp_theorique").innerHTML);}
	if ($("toto_chq_theorique").innerHTML != "") { toto_theo += toto_chq_theo = parseFloat($("toto_chq_theorique").innerHTML);}
	if ($("toto_cb_theorique").innerHTML != "") { toto_theo += toto_cb_theo = parseFloat($("toto_cb_theorique").innerHTML);}
	...
	if (toto_esp_saisie-toto_esp_theo >0) {$("diff_esp").innerHTML = $("diff_esp2").innerHTML = "+"+(toto_esp_saisie-toto_esp_theo).toFixed(tarifs_nb_decimales);}
	...
	if (toto_chq_saisie-toto_chq_theo >0) {$("diff_chq").innerHTML = $("diff_chq2").innerHTML = "+"+(toto_chq_saisie-toto_chq_theo).toFixed(tarifs_nb_decimales);}
	$("diff_cb").innerHTML = $("diff_cb2").innerHTML = $("montant_erreur_cb").value = (toto_cb_saisie-toto_cb_theo).toFixed(tarifs_nb_decimales);
	if (toto_cb_saisie-toto_cb_theo >0) {$("diff_cb").innerHTML = $("diff_cb2").innerHTML = "+"+(toto_cb_saisie-toto_cb_theo).toFixed(tarifs_nb_decimales);}
	...
	} else {$("commentaire_add").innerHTML = "";}
...
function calcul_transfert_caisse () {
	...
	if (toto_esp_theo-toto_esp_saisie >0) {$("diff_esp").innerHTML = "+"+(toto_esp_theo-toto_esp_saisie).toFixed(tarifs_nb_decimales);}
	$("diff_chq").innerHTML = (toto_chq_theo-toto_chq_saisie).toFixed(tarifs_nb_decimales);
	if (toto_chq_theo-toto_chq_saisie >0) {$("diff_chq").innerHTML = "+"+(toto_chq_theo-toto_chq_saisie).toFixed(tarifs_nb_decimales);}
...
array_id.each(function(j) {
								if(j!=undefined) {
									if ($(j[0])) {
										$(j[0]).style.display="none";
										if ($(j[1]+"_2")) {	$(j[1]+"_2").className="chemin_numero_gris";}
										if ($(j[1]+"_3")) {	$(j[1]+"_3").className="chemin_texte_gris";}
									}
								}
							}
						);
...
function	controle_caisse_historique_result () {
	id_move_type = $("id_move_type").getValue().join(", ");
		var AppelAjax = new Ajax.Updater(
									...
									contentType:  'application/x-www-form-urlencoded',
									encoding:     'UTF-8',
									parameters: { recherche: '1', id_compte_caisse: $("choix_id_caisse").value, id_move_type: id_move_type, page_to_show: $("page_to_show_s").value , date_debut: $("date_debut").value, date_fin: $("date_fin").value},
									evalScripts:true, 
									onLoading:S_loading, onException: function () {S_failure();}, 
									onComplete:H_loading
									}
...
function	compta_journal_achats_result () {
		var AppelAjax = new Ajax.Updater(
									...
									contentType:  'application/x-www-form-urlencoded',
									encoding:     'UTF-8',
									parameters: { recherche: '1',  page_to_show: $("page_to_show_s").value , date_debut: $("date_debut").value, date_fin: $("date_fin").value, ref_contact: $("ref_contact").value, numero_compte: $("numero_compte").value},
									evalScripts:true, 
									onLoading:S_loading, onException: function () {S_failure();}, 
									onComplete:H_loading
									}
...
function	compta_journal_achats_result_byexercice () {
	$("date_debut").value=""; $("date_fin").value="";
		var AppelAjax = new Ajax.Updater(
									...
									contentType:  'application/x-www-form-urlencoded',
									encoding:     'UTF-8',
									parameters: { recherche: '1',  page_to_show: $("page_to_show_s").value ,  date_exercice: $("date_exercice").value, ref_contact: $("ref_contact").value, numero_compte: $("numero_compte").value},
									evalScripts:true, 
									onLoading:S_loading, onException: function () {S_failure();}, 
									onComplete:H_loading
									}
...
function	compta_journal_tresorerie_result () {
		var AppelAjax = new Ajax.Updater(
									...
									contentType:  'application/x-www-form-urlencoded',
									encoding:     'UTF-8',
									parameters: { recherche: '1', id_journal: $("id_journal").value,  page_to_show: $("page_to_show_s").value , date_debut: $("date_debut").value, date_fin: $("date_fin").value, ref_contact: $("ref_contact").value},
									evalScripts:true, 
									onLoading:S_loading, onException: function () {S_failure();}, 
									onComplete:H_loading
									}
...
function	compta_journal_tresorerie_result_byexercice () {
	$("date_debut").value=""; $("date_fin").value="";
		var AppelAjax = new Ajax.Updater(
									...
									contentType:  'application/x-www-form-urlencoded',
									encoding:     'UTF-8',
									parameters: { recherche: '1', id_journal: $("id_journal").value,  page_to_show: $("page_to_show_s").value ,  date_exercice: $("date_exercice").value, ref_contact: $("ref_contact").value},
									evalScripts:true, 
									onLoading:S_loading, onException: function () {S_failure();}, 
									onComplete:H_loading
									}
...
function	compta_situation_commerciaux_result () {
		var AppelAjax = new Ajax.Updater(
									...
									contentType:  'application/x-www-form-urlencoded',
									encoding:     'UTF-8',
									parameters: { recherche: '1',  page_to_show: $("page_to_show_s").value , date_debut: $("date_debut").value, date_fin: $("date_fin").value},
									evalScripts:true, 
									onLoading:S_loading, onException: function () {S_failure();}, 
									onComplete:H_loading
									}
...
function	compta_situation_commerciaux_result_byexercice () {
	$("date_debut").value=""; $("date_fin").value="";
		var AppelAjax = new Ajax.Updater(
									...
									contentType:  'application/x-www-form-urlencoded',
									encoding:     'UTF-8',
									parameters: { recherche: '1',  page_to_show: $("page_to_show_s").value ,  date_exercice: $("date_exercice").value},
									evalScripts:true, 
									onLoading:S_loading, onException: function () {S_failure();}, 
									onComplete:H_loading
									}
