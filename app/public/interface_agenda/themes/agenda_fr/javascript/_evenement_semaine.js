...
EvenementSemaine.prototype = Object.extend(new Evenement(), {
	...
	// COMMUNICATION
	//###########################################################################
	showOnPanel : function(demanderPermission){
		//debugger;
		if(demanderPermission == null || (demanderPermission && (this.gestionnaireEvenement == null || this.gestionnaireEvenement.showOnPanel(this)))){
			var Udate_even  =Udate_deb_semaine + this.cellJour*86400000 + this.cellHeurDeb*1800000;
			var duree_event = Math.ceil(((this.height * 30) / HAUTEUR_DEMIE_HEURE)/15) * (900);
			//		Math.ceil(((  this.height * DemieHeure) / HAUTEUR_DEMIE_HEURE)/15) * (15*UNE_MINUTE);
			showOnPanel("SEMAINE", this.id, this.ref_event, Udate_even, duree_event);
