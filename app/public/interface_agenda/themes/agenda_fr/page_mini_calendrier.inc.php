...
<table width="156px" border="0" cellpadding="0" cellspacing="0" height="100%">
  <thead>
  ...
  <tbody>
  	<tr>
			...
			while ($Udate_tmp < $Udate_fdm){
				...
				else{	echo strftime('<td class="mini_calendrier_Jsemaine_chiffre_hors_mois" onclick="refresh_grille_agenda('.($Udate_tmp).'000);" >%d</td>', $Udate_tmp);}
			...
			while ($j < 7) { 
				...
				else{	echo strftime('<td class="mini_calendrier_Jsemaine_chiffre" onclick="refresh_grille_agenda('.($Udate_tmp).'000);" >%d</td>', $Udate_tmp);}
				$Udate_tmp = $Udate_tmp + 86400 ;
				$j++;
			}?>
  	</tr>
