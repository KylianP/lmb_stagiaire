	...
	<td width="15px">
		...
		foreach ($catalogues_clients as $catalogue_client) {
			...
			<div style="width:180px; text-align: center">
				<input type="checkbox" id="ins_<?php echo $catalogue_client->id_catalogue_client;?>_<?php echo $indentation_case_a_cocher?>" name="ins_<?php echo $catalogue_client->id_catalogue_client;?>_<?php echo $indentation_case_a_cocher?>"  <?php
			...
			if ($('ins_<?php echo $catalogue_client->id_catalogue_client;?>_<?php echo $indentation_case_a_cocher?>').checked) {
			 add_catalogue_client_dir("<?php echo $catalogue_client->id_catalogue_client;?>", "<?php echo ($art_categ->ref_art_categ)?>", "<?php echo ($art_categ->ref_art_categ_parent)?>");
			} else {
				del_catalogue_client_dir("<?php echo $catalogue_client->id_catalogue_client;?>", "<?php echo ($art_categ->ref_art_categ)?>");
			}
			}, false);
			</SCRIPT>
			</div>
				...
				<table cellpadding="0" cellspacing="0" width="5px">
					...
					foreach ($catalogues_clients as $catalogue_client) {
						...
						<div style="display:block; width:180px; text-align: center">
							...
							<span id="decoche_all_<?php echo $catalogue_client->id_catalogue_client;?>" style="cursor:pointer">Tout décocher</span>
							<script type="text/javascript">
							Event.observe('coche_all_<?php echo $catalogue_client->id_catalogue_client;?>', 'click',  function(evt){
								add_all_art_categ_to_catalogue ("<?php echo $indentation_case_a_cocher;?>", "<?php echo $catalogue_client->id_catalogue_client;?>");
							}, false);
							Event.observe('decoche_all_<?php echo $catalogue_client->id_catalogue_client;?>', 'click',  function(evt){
								del_all_art_categ_to_catalogue ("<?php echo $indentation_case_a_cocher;?>", "<?php echo $catalogue_client->id_catalogue_client;?>");
							}, false);
							</script>
							</div>
						</td>
