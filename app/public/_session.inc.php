...
if (!function_exists  ("__autoload")){
	function __autoload($classname){
		...
		if(class_exists($classname)){return true;} // La classe est d�j� charg�e.
...
$_ENV['CHEMIN_ABSOLU'] = "http://".$_SERVER['HTTP_HOST'].rtrim(dirname($_SERVER['PHP_SELF']), '/\\')."/".$DIR;
...
require_once ($DIR."_fonctions_generales.inc.php");		// Fonctions g�n�rales
require_once ($DIR."_annuaire.lib.php");							// Fonctions li�es � l'annuaire
require_once ($DIR."_catalogue.lib.php");							// Fonctions li�es au catalogue
require_once ($DIR."_document.lib.php");
require_once ($DIR."_tarif.lib.php");									// Fonctions li�es aux tarifs
require_once ($DIR."_divers.lib.php");								// Fonctions diverses
require_once ($DIR."_referencement.lib.php");					// Fonctions li�es au r�f�rencement
require_once ($DIR."_panier.lib.php");								// Fonctions li�es aux paniers clients
...
// Classes charg�es dynamiquement selon l'installation et la configuration du serveur.
...
// *************************************************************************************************************
$bdd = new PDO_etendu("mysql:host=".$bdd_hote."; dbname=".$bdd_base.";charset=latin1", $bdd_user, $bdd_pass);

// Afficher les erreurs PDO (li�es � la base de donn�e)
...
//traitement du register_globals � on
...
//mise � jour des donn�es de session si le fichier _session_maj a �t� modifi�
if (isset($_SESSION['user']) && $_SESSION['user']->getLogin()) {
	// ini_set("memory_limit","12M");
	// recup�ration du fichier t�moin de mise � jour pour comparaison et remise � zero des informations de session.
...
if (!isset($_SESSION['profils'])) {
	...
	// Chargement des profils en donn�e de session
...
if (!isset($_SESSION['magasins'])) {
	...
	$result = $bdd->query($query);
	// Chargement des magasins en donn�e de session
...
if (!isset($_SESSION['stocks'])) {
	...
	$query = "SELECT id_stock, lib_stock, abrev_stock, ref_adr_stock, actif
				...
				ORDER BY lib_stock";
	$result = $bdd->query($query);
	// Chargement des stocks en donn�e de session
...
if (!isset($_SESSION['types_docs'])) {
	...
	$result = $bdd->query($query);
	// Chargement des types de document en donn�e de session
...
// 	if (@remote_file_exists ($MAJ_SERVEUR['url']."check_version_dispo.php?version_actuelle=".$_SERVER['VERSION'])) {
...
// La page que consulte l'utilisateur est elle accessible � tous ?
if ((isset($_INTERFACE['MUST_BE_LOGIN']) && $_INTERFACE['MUST_BE_LOGIN']) || (isset($_PAGE['MUST_BE_LOGIN']) && $_PAGE['MUST_BE_LOGIN'])) {
	if (!$_SESSION['user']->getLogin()) {
		if ($_INTERFACE['ID_PROFIL'] == 4 && isset($THIS_DIR)) {
			...
			header ("Location: ".$_ENV['CHEMIN_ABSOLU'].$THIS_DIR."_user_login.php");
		} else {
			// L'utilisateur n'est pas loggu� et devrait l'etre => Direction page de login
...
// V�rifie si il y a changement d'interface
if ($_INTERFACE['ID_INTERFACE'] != $_SESSION['user']->getId_interface()) {
	// Si l'utilisateur n'a pas le droit d'acc�der � cette interface
	if (!$_SESSION['user']->interface_is_allowed ($_INTERFACE['ID_INTERFACE'])) {
		// Si il n'est pas identifi�, renvoi vers la page de login
		if (!$_SESSION['user']->getLogin()) {
			$page_from = substr($_SERVER['REQUEST_URI'], 1);
			header ("Location: ".$_ENV['CHEMIN_ABSOLU']."site/__user_login.php?page_from=".$page_from);
			exit();
		}
		// Si il est identifi�, il n'a tout simplement pas le droit
		...
		//raffraichissement forc� de la session
...
// Profil n�cessaire pour consulter la page en cours
...
if ($_SESSION['user']->getLogin() && ($_INTERFACE['ID_INTERFACE'] == 2 || $_INTERFACE['ID_INTERFACE'] == 3)) {
	// recup�ration du fichier t�moin de mise � jour pour comparaison et remise � zero des informations en cache.
...
// *************************************************************************************************************
// CRONS, a d�localiser au niveau du syst�me !!
// ***************************************************************************************************************
