...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#0099CC">
	<tr class="colorise1">
		...
		<td style=" width:10%">
			<div >
			<!--<a href="catalogue_article_view.php?ref_article=<?php echo $contenu->article->getRef_article();?>&" style="text-decoration:none; color:#000000; font-size:9px" id="link_to_art_<?php echo $indentation_contenu?>">-->
			<?php if ($contenu->article->getRef_interne() != "") { echo $contenu->article->getRef_interne();} else { echo $contenu->article->getRef_article();} ?><!--</a>-->
		...
		<td style="text-align:center; width:10%" >
			<div style=" text-align:center; margin:0 30% " >
			...
			<input value="<?php echo $contenu->qte;?>" id="qte_<?php echo $indentation_contenu?>" name="qte_<?php echo $indentation_contenu?>" type="text" size="2" class="input_add_panier" style="text-align:center;" />
					...
					<td style="text-align:center; width:15px;"><a href="#" id="qte_add_one_<?php echo $indentation_contenu;?>"><img src="<?php echo $DIR.$_SESSION['theme']->getDir_theme()?>images/ajouter.gif" /></a></td>
				</tr>
				<tr>
					<td style="text-align:center; width:15px;"><a href="#" id="qte_sub_one_<?php echo $indentation_contenu;?>"><img src="<?php echo $DIR.$_SESSION['theme']->getDir_theme()?>images/moins.gif" /></a></td>
				</tr>
			</table>
			</div>
		</td>	
		<td style="width:10%; text-align: center" >
			...
			<?php
			$stock = $contenu->article->getStocks();
				if (isset($stock[$_SESSION['magasins'][$ID_MAGASIN]->getId_stock()]) && $stock[$_SESSION['magasins'][$ID_MAGASIN]->getId_stock()]->qte) { ?><img src="<?php echo $DIR.$_SESSION['theme']->getDir_theme()?>images/stock_dispo.gif" /><?php } else { ?><img src="<?php echo $DIR.$_SESSION['theme']->getDir_theme()?>images/stock_vide.gif" /><?php }?>
			</a>
		</td>
		<td style="width:15%; text-align:right;">
			...
			if ($_SESSION["panier_interface_".$_INTERFACE['ID_INTERFACE']]["app_tarifs"] == "HT") {
				...
				echo number_format(( interface_article_pv ($contenu->article, $contenu->qte)*(1+$contenu->article->getTva()/100)), $TARIFS_NB_DECIMALES, ".", ""	)."&nbsp;".$MONNAIE[1];
			}
			}
			?>
		</td>	
